...
function StatCard({ icon: Icon, label, value, sub, loading, color = "#3981FA" }) {
  const [displayValue, setDisplayValue] = useState(0);
  const [isHovered, setIsHovered] = useState(false);
  ...
  return (
    <div
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
      className="group relative overflow-hidden bg-white rounded-2xl p-6 flex flex-col justify-between min-h-[145px] transition-all duration-300 -translate-y-1 select-none hover:-translate-y-2 cursor-pointer"
      style={{
        border: `1px solid ${color}30`,
        boxShadow: isHovered
          ? `0 20px 40px ${color}25, 0 4px 12px ${color}15`
          : `0 4px 16px ${color}10`,
      }}
    >
      {/* Ambient glow top-left */}
      <div
        className="absolute -top-10 -left-10 w-32 h-32 rounded-full pointer-events-none blur-2xl transition-all duration-500"
        style={{
          background: `${color}08`,
      ...
      {/* BG icon watermark */}
      <div
        className="absolute -right-4 -bottom-6 pointer-events-none transform origin-bottom-right transition-all duration-500"
        style={{
          color: `${color}18`,
          transform: isHovered
            ? "scale(2.6) rotate(-25deg)"
            : "scale(2.4) rotate(-12deg)",
        }}
      >
      ...
      <div className="flex items-center justify-between gap-3 relative z-10">
        <div className="flex flex-col gap-0.5">
          <p className="text-[11px] font-bold tracking-widest text-[#6A7281] uppercase">
            {label}
          </p>
          <p className="text-[11px] text-[#9AA5B4] font-medium">{sub}</p>
        </div>
        ...
        >
          <Icon
            ...
            className="text-white transition-all duration-300"
            style={{
              transform: isHovered ? "scale(1.25) rotate(12deg)" : "scale(1) rotate(0deg)",
            }}
          />
        </div>
      </div>
          ...
          <div className="flex items-baseline gap-1.5">
            ...
            <span
              ...
              style={{
                background: color,
                transform: isHovered ? "scale(1.5)" : "scale(1)",
                boxShadow: isHovered ? `0 0 6px ${color}80` : "none",
              }}
            />
          </div>
        )}
      ...
      {/* Left accent stripe */}
      <div
        className="absolute top-0 bottom-0 left-0 rounded-l-2xl transition-all duration-300"
        style={{
          width: isHovered ? "4px" : "3px",
          background: `linear-gradient(to bottom, ${color}, ${color}40)`,
          boxShadow: isHovered ? `2px 0 8px ${color}40` : "none",
        }}
      />
