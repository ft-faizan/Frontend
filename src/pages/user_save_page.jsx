...
function User_save_page() {
  ...
  useEffect(() => {
    if (location.state?.activeTab) {
      ...
      // Clean up the history state context so a page refresh doesn't force-lock the tab
        ...
        <div className="flex gap-1 bg-white rounded-xl p-1 w-fit mb-7">
          {tabs.map((tab) => (
            <button
              ...
              className={`px-5  py-2 rounded-lg text-sm font-medium transition-all cursor-pointer ${activeTab === tab.id ? "bg-[#3E81F4] text-white font-bold" : "text-gray-500 hover:text-[#2F70EB]"}`}
            >
              {tab.label}
            </button>
          ))}
